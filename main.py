...
for retry in range(0, MAX_RETRIES): 
    ...
    if response.ok:
        ...
        print("Connect failed with the url!" + url1)
...
for retry in range(0, MAX_RETRIES):
    ...
    if response.ok:
        ...
        print("Connect failed with the url!" + url2)
        attempts_left = MAX_RETRIES - retry - 1
        if (attempts_left != 0):
            print(str(attempts_left) + " attempts left!")
        else: 
...
finder = BS(response.content,"lxml")
url_logout = finder.find_all("a")[22].attrs["href"]
json_url = url_logout.replace("https://e-learning.hcmut.edu.vn/login/logout.php?sesskey=", "https://e-learning.hcmut.edu.vn/lib/ajax/service.php?sesskey=") + "&info=core_message_get_conversation_counts,core_message_get_unread_conversation_counts"
...
for retry in range(0, MAX_RETRIES):
    ...
    if response.ok:
        ...
        if (attempts_left != 0):
            ...
            print("Pass over the log out url!")
            break
